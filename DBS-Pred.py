...
dat2 = pd.read_excel('mappin3.xlsx', index_col=0)
...
dat3 = pd.read_excel('dataJAN.xlsx', index_col=0)
...
dat1_filt['Race'] = dat1_filt['Race'].replace({'white': 0, 'white ': 0, 'native american or alaska native/ white': 0, 'hispanic': 1, 'asian': 2, 'black': 3, 'black or african american': 3})
...
billist = np.array(dat1_filt["No_Leads"].replace({'Bi': 0, 'BI ': 0, 'BI': 0, 'Bi ': 0, 'L': 1, 'R' : 1, 'Bi STN\nL GPi\n(3 total)' : 0, "Bi (4 total)" : 0 }))
retrclin = np.array(dat1_filt["PTID_Retro_Clin"])
race = np.array(dat1_filt['Race'])
ids = np.array(dat3_cl["PTID_Retro_Clin"])
#%%
# Remove NaNs
