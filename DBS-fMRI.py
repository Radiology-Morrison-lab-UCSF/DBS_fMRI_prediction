...
dat2 = pd.read_excel('mappin2.xlsx', index_col=0)
index_values = dat2.index
dat2["PTID_Retro_Clin"] = index_values
id_map = dat2.set_index('PTID_Retro_Clin')['PTID_CONN'].to_dict()

# Load main data file with clinical info
dat3 = pd.read_excel('dataOCT.xlsx', index_col=0)
index_values = dat3.index
dat3["PTID_Retro_Clin"] = index_values
dat3['PTID_CONN'] = dat3['PTID_Retro_Clin'].map(id_map)
dat3_cl = dat3.dropna(subset=['PTID_CONN'])
dat3_cl['PTID_CONN'] = pd.to_numeric(dat3_cl['PTID_CONN'], errors='coerce')
dat3_cl = dat3_cl[pd.notna(dat3_cl['PTID_CONN'])]
dat3_cl["PTID_CONN"] = dat3_cl["PTID_CONN"].astype(int)
dat3_cl.sort_values(by='PTID_CONN', ascending=True, inplace=True)

# Load functional data
data = loadmat('allrois_new.mat')
data2 = loadmat('allrois_fc_ctrl.mat')
dataht = loadmat('allroisfc_ht.mat')
mssd = loadmat('mssdresults')
fcvar = loadmat('allrois_fc_var')

func = data['allrois_fc']
funcctrl = data2['allrois_fc']
funcht = dataht['allrois_fc']
mssd = mssd['mssdResults'][0]
fcvar = fcvar['allrois_fc']
...
for i in range(len(post1_raw)):
    if np.isnan(post1_raw[i]) and np.isnan(post2_raw[i]) and np.isnan(post3_raw[i]) and np.isnan(post4_raw[i]):
        ...
        if not np.isnan(post1_raw[i]):
            improvs.append(post1_raw[i])
        elif not np.isnan(post2_raw[i]):
            improvs.append(post2_raw[i])
        elif not np.isnan(post3_raw[i]):
            improvs.append(post3_raw[i])
        elif not np.isnan(post4_raw[i]):
...
# Interpolate NaNs in MDS-UPDRS if any
if np.where(np.isnan(mds))[0].size > 0:
...
dat1_filt = dat3_cl.copy()
dat1_filt['DOB'] = pd.to_datetime(dat1_filt['DOB'])
dat1_filt['LEDDpre_Date'] = pd.to_datetime(dat1_filt['LEDDpre_Date'])
ages_in_days = (dat1_filt['LEDDpre_Date'] - dat1_filt['DOB']).dt.days
dat1_filt['age'] = ages_in_days / 365.25
...
dat1_filt['Target_L_R'] = dat1_filt['Target_L_R'].replace(target_map)

# Map Sex
dat1_filt['Sex'] = dat1_filt['Sex'].replace({'M': 0, 'F': 1})

yearspd = np.array(dat1_filt['Years_PD_at_SX_MRI'])
gender = np.array(dat1_filt['Sex'])
age = np.array(dat1_filt['age'])
target = np.array(dat1_filt['Target_L_R'])
billist = np.array(dat1_filt["No_Leads"])
retrclin = np.array(dat1_filt["PTID_Retro_Clin"])
...
# Remove a specific subject index (as in original code)
...
def lassoreg_coeffs(indp, dp, alpha):
    """
    Performs LASSO regression with a given alpha and returns the most significant predictors.

    Parameters
    ----------
    indp : np.ndarray
        Independent variables (features) array.
    dp : np.ndarray
    ...
    mostyy : list
        (row, column) pairs corresponding to top features.
    sig_cor : list
        Human-readable names of top connections.
    coefficients : np.ndarray
        LASSO model coefficients.
    conv : int
        Convergence flag (1 if converged, 0 if not).
    sortyy : list
        Indices of all non-zero coefficients.
    coefsstore : list
    ...
    sorty2 = [idx for idx in sorty if idx < len(z_func_s[0])]
    mosty = np.array(sorty2[:10])

    # Map top features back to (row, col)
    origshape = (36, 36)
    mostyy = []
    ...
    return mosty, mostyy, sig_cor, coefficients, conv, store, coefsstore
...
def test_r2_tott(indp, dp):
    ...
    return significant_indices, sorted_p_values, predictions, residuals, aic, adjusted_r_squared, summary
#%%

def generate_train_test_indices(num_sets, set_size, max_value):
    """
    Generates multiple random index subsets for cross-validation or bootstrapping,
    including complementary test sets.
    ...
    indices_list : list of tuples
        ...
        - Training indices (size `set_size`).
        - Test indices (complementary set).
    """
    indices_list = []
    for _ in range(num_sets):
        train_indices = np.random.choice(range(max_value), set_size, replace=False)
        test_indices = np.setdiff1d(range(max_value), train_indices)
...
def test(alpha, indp, dp, fc, comb=1, alll=1):
    ...
    i = 0
    test_indices = index_sets[2500:];
    
    for elem, test_index in zip(index_sets[:2500], test_indices):
        i += 1
        ...
        # Lasso Regression
        _, lassoreg, _, _, _,lasstore, coefsstore = lassoreg_coeffs(x_train, y_train, alpha) 
        ...
        _, _, _, _, _, adjlas, summary = test_r2_tott(indp_fl_las_s, y_test)
...
for i in range(100):
    ...
    _, _, _, _, _, adj_r2_ho, summary = test_r2_tott(X_ho_final_s, dp_s_ho)
    adj_r2_ho_tot.append(adj_r2_ho)
...
pairs_in_36 = np.array(final_set).copy()
...
for i in range(len(final_set)):
    sig_cor.append(names[final_set[i][0]] + ' and ' + names[final_set[i][1]])
    sig_con.append([final_set[i][0], final_set[i][1]])

#%% Get features for PD, HC and HD groups
_, featslas = feats_uniq(final_set, funcready)
...
data = [rsq_las_01_cl, rsq_las_01]
labels_box2 = ['Clinical', 'Clinical + fMRI']

t_stat, p_value = ttest_ind(rsq_las_01_cl, rsq_las_01, equal_var=False)
...
y_max = max(np.max(rsq_las_01_cl), np.max(rsq_las_01))
h = 0.02
y = y_max * 1.05
x1, x2 = 0, 1
ax.plot([x1, x1, x2, x2], [y, y+h, y+h, y], lw=1.5, c='k')
...
allalp = [rsq_las_0001, rsq_las_0005, rsq_las_001, rsq_las_005, rsq_las_007, rsq_las_01, rsq_las_02, rsq_las_05, rsq_las_1]
