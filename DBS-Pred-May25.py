...
withcov = np.concatenate((z_func_s, mds, agee, genderr, targett, billist), axis=1).astype(float)
...
#example_dict = lasso
#result = find_frequent_pairs(result_alliter)
#print(len(result))
...
def generate_indices(num_sets, set_size, max_value):
    ...
    return indices_list
#%% Use this for comparing different models and LASSO penalty
...
def test(alpha, indp, dp, fc, comb=1, alll=1):
    ...
    for elem, test_index in zip(index_sets[:5000], test_indices):
        ...
        x_train, _ = indp[elem], indp[test_index]
        ...
        billist_train, billist_test = billist[elem], billist[test_index]
        # Lasso Regression
        ...
        if comb == 1:
            indp_fl_las = np.concatenate((featslas, mds[test_index], age_test, gender_test, target_test, billist_test), axis=1).astype(float)
        else:
            indp_fl_las = np.concatenate((mds[test_index], age_test, gender_test, target_test, billist_test), axis=1).astype(float)
        ...
        if alll == 1:
            ...
            indp_fl_svm = np.concatenate((featssvm, age_test, gender_test, target_test, billist_test), axis=1).astype(float)
            scaler_X = StandardScaler()
            ...
            indp_fl_rfe = np.concatenate((featsrfe, age_test, gender_test, target_test, billist_test), axis=1) 
            ...
            indp_fl_rfs = np.concatenate((featsrfs, age_test, gender_test, target_test, billist_test), axis=1) 
...
#%% Use this for testing extracted features' robustness and hold-out testing

all_indices = np.arange(120)  # Assuming total subjects are 120
adj_r2_ho_tot_01 = [] #for the penalty 0.1
result_alliter = {}
siginds = []
pvals = []
coefs = []
for i in range(100):
    ...
    hold_out_indices = np.random.choice(all_indices, size=30, replace=False)
    ...
    billistt_ho = billist[hold_out_indices]
...
pairs_in_36 = np.array(result).copy()
...
for i in range(len(result)):
    sig_cor.append(names[result[i][0]] + ' and ' + names[result[i][1]])
    sig_con.append([result[i][0], result[i][1]])
...
withcov_ts = np.concatenate((featslas, mds, agee, genderr, targett, billist), axis=1).astype(float)
scaler_x = StandardScaler()
withcov_ts_s = scaler_y.fit_transform(withcov_ts)

significant_indices, sorted_p_values, predictions, residuals, aic, adjlas, summary, pvals = test_r2_tott(withcov_ts_s, dp_s)
...
connections = ['Cerebelum-6 r and Postcentral l',
 'Cerebelum-3 r and Dentate-Nucleus l',
 'Putamen l and Cerebelum-45 r',
 'Cerebelum-3 l and Cerebelum-45 l',
 'Dentate-Nucleus r and Red-Nucleus l',
 'Ventral-Pallidum l and Postcentral l',
 'Precentral l and Supp-Motor-Area r',
 'TL-Anterior l and Subthalamic-Nucleus l']

coefficients = [ 0.28530424,  0.09877667, -0.15637469,  0.11871166, -0.27610043,
        0.22553815,  0.22984748, -0.17639923]
...
region_to_index = {
 ...
 'Postcentral l': 8,
 'Precentral l': 9,
 'Supp-Motor-Area r': 10,
 'Subthalamic-Nucleus l': 11,
 'Red-Nucleus l': 12,
 'Dentate-Nucleus r': 13,
 'TL-Anterior l': 14
